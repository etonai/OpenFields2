...
### 3. Melee Combat Test Fix ⭕ **PLANNING**
...
## System 3 Implementation Plan (Based on User Answers)

### Phase 1: Revert AutoTargetingSystem Changes
- **Remove the instanceof check I added** - confirmed not needed based on manual testing
- The original AutoTargetingSystem code works properly with melee weapons

### Phase 2: Fix Test Configuration (Priority Order)
**Step 1 (First Priority): Restore auto-targeting in test code**
- Remove `soldierAlpha.usesAutomaticTargeting = false` and `soldierBeta.usesAutomaticTargeting = false`
- Follow GunfightTestAutomated model for auto-targeting setup
- Remove manual `alphaUnit.setTarget()` commands that prevent auto-targeting

**Step 2 (Second Priority): Fix save file configuration**
- Model test_d.json after test_b.json used by GunfightTestAutomated
- Key difference: use melee characters/weapons instead of ranged
- Ensure save file has proper auto-targeting configuration

**Step 3: Restore proper success criteria**
- Original criteria: "no exceptions AND at least one defense attempt"
- **Enhanced criteria**: Add validation that at least one attack is performed
- Keep enhanced debug monitoring for validation

### Phase 3: Test Verification Strategy
- **Make all changes simultaneously** then test together
- **Primary verification**: Run test and check for `[DEFENSE]` messages in output
- **Success indicators**: Defense attempts > 0 AND attack attempts > 0
- **Follow GunfightTestAutomated patterns** for all test configuration aspects
