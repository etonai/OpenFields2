...
*Created: 2025-06-25 | Last Design Update: 2025-06-25 | Last Implementation Update: 2025-06-25 | Implementation Status: COMPLETED*
...
#### Bug #1: Rapid Consecutive Melee Attacks
...
**Root Cause**: Likely related to the weapon state transition bug that was fixed (melee_attacking → sheathed instead of melee_ready). Multiple attack events may be getting scheduled when the weapon state is incorrectly managed.
...
**Fix Required**: Ensure proper attack recovery timing and prevent new attacks from being scheduled until the previous attack's recovery period completes.

#### Bug #2: Defense System Not Triggering for All Characters
...
**Root Cause**: Likely an issue in `CombatResolver.resolveMeleeAttack()` where the defense check (`target.character.canDefend(attackTick)`) is not being called or is returning false for certain characters.

**Priority**: High - Defense system is a core feature of DevCycle 23

**Location**: `CombatResolver.java` lines 369-376, `Character.canDefend()` method

**Fix Required**: 
1. Investigate why `canDefend()` check is failing for some characters
2. Ensure defense attempts trigger consistently for all valid targets
3. Add additional debug logging to track defense system activation
