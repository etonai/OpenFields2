...
**Description:**
When a character with automatic targeting enabled moves into melee range of a target, the character successfully moves to the target but does not automatically initiate a melee attack. The character reaches the target and stops, but the attack sequence is not triggered.

**Expected Behavior:**
- Character with auto-targeting enabled moves toward hostile target
- When character reaches melee range, automatic attack should be triggered
- Attack should execute using the currently equipped melee weapon
- Process should be seamless and automatic like ranged auto-targeting

**Root Cause Analysis:**
The automatic targeting system appears to handle movement to melee range correctly, but lacks the integration point to trigger melee attacks when the target comes within range. The auto-targeting logic may be missing the melee attack initiation step.

**Implementation Plan:**
1. **Investigate Auto-Targeting Integration** (Day 1)
   - Review `Character.updateAutomaticTargeting()` method for melee attack triggers
   - Check `Character.updateMeleeMovement()` integration with auto-targeting
   - Identify missing connection point between range detection and attack initiation

2. **Implement Melee Attack Trigger** (Day 1-2)
   - Add melee attack initiation when auto-targeting character reaches melee range
   - Ensure proper weapon mode checking (melee vs ranged) for auto-targeting
   - Integrate with existing `scheduleMeleeAttackFromCurrentState()` method

3. **Test and Validate** (Day 2)
   - Test auto-targeting behavior with various melee weapons and ranges
   - Verify seamless transition from movement to attack
   - Ensure compatibility with existing ranged auto-targeting
...
1. **Analyze Current Damage Calculation** (Day 1)
   ...
   - Check strength modifier application in melee damage
   - Verify wound severity impact on actual damage dealt
   - Compare with ranged weapon damage for consistency

2. **Fix Damage Formula** (Day 1-2)
   - Correct excessive damage multipliers
   - Ensure strength modifiers are reasonable (+/-20% range suggested)
   - Verify wound severity affects hit location/healing, not base damage
   - Balance melee damage to be comparable with ranged weapons
...
- Steel Dagger (base 6): Light wound should deal 6-8 damage, not 11
- Strength modifiers should add +/-20% maximum (1-2 points for dagger)
- Critical wounds should be higher damage, not light wounds
...
### Day 1: Analysis and Quick Fixes
- [ ] **Morning**: Analyze auto-targeting integration for melee attacks
- [ ] **Afternoon**: Update JSON files with projectile names for all melee weapons

### Day 2: Core Fixes
- [ ] **Morning**: Fix melee damage calculation and test with various scenarios
- [ ] **Afternoon**: Implement melee-specific combat messages
