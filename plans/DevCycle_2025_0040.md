...
### 5. Incapacitation Bug Fix ⭕ **PLANNING**
...
- [ ] **Incapacitation Logic Analysis**
  - [ ] Investigate Character.isIncapacitated() method implementation
  - [ ] Check when and how incapacitation status is determined during combat
  - [ ] Verify incapacitation check timing in melee combat flow
  - [ ] Analyze damage application vs incapacitation check order

- [ ] **Combat System Integration**
  - [ ] Ensure incapacitation checks occur immediately after damage application
  - [ ] Verify combat systems respect incapacitation status and stop further attacks
  - [ ] Check auto-targeting system response to incapacitated targets
  - [ ] Validate that incapacitated characters cannot continue attacking

- [ ] **Damage Flow Correction**
  - [ ] Fix timing of incapacitation checks in damage application sequence
  - [ ] Ensure damage application stops when character becomes incapacitated
  - [ ] Prevent further wound accumulation after incapacitation
  - [ ] Test edge cases around simultaneous attacks and incapacitation
...
**Design Specifications:**
- **Immediate Incapacitation**: Characters must be incapacitated immediately when health ≤ 0
- **Combat Cessation**: All combat actions must stop for incapacitated characters
- **Damage Prevention**: No further damage accumulation after incapacitation
- **Auto-Targeting Response**: Auto-targeting must recognize and respond to incapacitated targets

**Technical Implementation Notes:**
- **Key Files to Investigate**: `Character.java` (incapacitation logic), melee combat damage flow, CombatCoordinator
- **Root Issue**: Timing or logic error in incapacitation checking during combat damage sequence
- **Solution Pattern**: Ensure incapacitation check occurs immediately after each damage application
- **Validation**: Characters stop taking damage and participating in combat at health ≤ 0

### 6. Duplicated Skills Bug Fix ⭕ **PLANNING**
...
- [ ] **Skills Data Structure Analysis**
  - [ ] Investigate Character.getSkills() method and underlying data structure
  - [ ] Check if skills are being added multiple times during character loading
  - [ ] Verify skills data integrity in save files and character creation
  - [ ] Analyze DisplayCoordinator skills display implementation

- [ ] **Character Loading Investigation**
  - [ ] Check character loading from JSON/save files for duplicate skill addition
  - [ ] Investigate faction character creation and skill assignment process
  - [ ] Verify test character setup doesn't duplicate skills during initialization
  - [ ] Analyze CharacterData deserialization for skill duplication

- [ ] **Display System Correction**
  - [ ] Fix skills display logic to show each skill only once
  - [ ] Ensure DisplayCoordinator properly iterates through skills collection
  - [ ] Validate skills collection doesn't contain duplicates at source
  - [ ] Test skills display with various character configurations
