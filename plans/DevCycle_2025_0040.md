...
- [ ] **Test Class Creation**
  - [ ] Create new `MeleeCombatTestAutomated` class following `GunfightTestAutomated` pattern
  - [ ] Implement automated test that runs with `mvn test`
  - [ ] Follow `GunfightTestAutomated` selection logic/timing for character management
  - [ ] Integrate with existing test framework infrastructure

- [ ] **Test Faction Enhancement**
  - [ ] Add knife skill (level 1) to TestFactionAlpha Soldier character (permanent)
  - [ ] Add knife skill (level 1) to TestFactionBeta Soldier character (permanent)
  - [ ] Ensure both soldiers have mel_bowie_knife as melee weapon
  - [ ] Set `isMeleeCombatMode = true` on both soldiers

- [ ] **Test Scenario Configuration**
  - [ ] Create new saves/test_d.json save file for melee combat test
  - [ ] Position soldiers 40 feet apart (280 pixels: 40 × 7 pixels/foot)
  - [ ] Configure both soldiers with `autoTargeting = true`
  - [ ] Ensure complete test isolation using test_d.json initialization
  - [ ] Implement 1-minute duration limit (3600 ticks) with 90-second timeout

- [ ] **Success Criteria Implementation**
  - [ ] Primary: Test passes if no exceptions thrown AND at least one defense attempt
  - [ ] Validation: Ensure characters approach each other and make at least one attack
  - [ ] No defense timing validation (nextDefenseTick cooldown) required

- [ ] **Validation Features (Development/Debugging)**
